...
class Rcon
{
    ...
    $rconHost = $_POST['rconHost'];
    $rconPort = (int) $_POST['rconPort'];
    $rconPassword = $_POST['rconPassword'];
	$timeout2 = $_POST['timeout'];


try {
    $rcon = new Rcon($rconHost, $rconPort, $rconPassword, $timeout2);
    if ($rcon->connect()) {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Failed to connect to RCON server.'
        ]);
    }
} catch (Exception $e) {
    echo json_encode([
